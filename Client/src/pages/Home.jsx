...
function Home() {
    ...
    const fetchEvents=async ()=>{
        try {
            ...
            setEvents(Array.isArray(data) ? data : data.events || data.data || []);        }  catch (error) {
    ...
    <div className="flex flex-col min-h-screen">
        ...
        {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {events.map(events => (
                    <div key={events._id} className="bg-white rounded-xl overflow-hidden shadow-md hover:shadow-xl transition flex flex-col">
                        <div className="h-48 bg-gray-200 overflow-hidden relative">
                            {events.image ? (
                                <img src={events.image} alt={events.title} className="w-full h-full object-cover" />
                            ) : (
                            ...
                            <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full text-sm font-bold shadow-sm">
                                {events.ticketPrice === 0 ? <span className="text-green-600">FREE</span> : <span className="text-gray-900">₹{events.ticketPrice}</span>}
                            </div>
                        </div>
                        <div className="p-6 grow flex flex-col">
                            <div className="text-xs font-bold text-gray-700 uppercase tracking-wider mb-2">{events.category}</div>
                            <h2 className="text-xl font-bold text-gray-800 mb-3">{events.title}</h2>
                            <div className="flex flex-col gap-2 mb-4 text-gray-600 text-sm">
                                <div className="flex items-center gap-2">
                                    <FaCalendarAlt className="text-gray-400" />
                                    <span>{new Date(events.date).toLocaleDateString(undefined, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</span>
                                </div>
                                <div className="flex items-center gap-2">
                            ...
                            <div className="mt-auto">
                                <div className="w-full bg-gray-200 rounded-full h-2 mb-2">
                                    <div className="bg-gray-700 h-2 rounded-full" style={{ width: `${(events.availableSeats / events.totalSeats) * 100}%` }}></div>
                                </div>
                                <p className="text-xs text-gray-500 mb-4">{events.availableSeats} of {events.totalSeats} seats remaining</p>
                                <Link to={`/events/${events._id}`} className="block w-full text-center bg-gray-100 hover:bg-gray-200 text-gray-900 font-semibold py-2 rounded-lg transition">
                                    View Details
                                </Link>
                            </div>
                        </div>
